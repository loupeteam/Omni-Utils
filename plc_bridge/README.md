...
| Rule | Meaning |
|---|---|
| Synchronous | Every method blocks. A driver on an async transport (websockets) owns its event loop and hides it. |
| Two caller threads | `read` comes from the read thread, `write` from the write thread, possibly at once. `connect` and `disconnect` only come from the read thread. The driver makes that safe: two connections, or a lock. |
| Stateless read list | The symbols arrive with each `read`. |
...
| Errors are not values | A symbol the PLC rejects goes in `ReadResult.errors`. A failed connection or request raises. |
...
`arr[2]` a list padded with `None`, `arr[2].x` a dict inside that list. This is
the parser the Beckhoff and B&R bridges each carried a copy of.
...
polling. Both threads are daemons, and `stop()` returns within two seconds even
when a read is stuck.
